...
mod tests {
    ...
    use halo2_proofs::{
        ...
        plonk::{create_proof, create_proof_leader, create_proof_worker, keygen_pk, keygen_vk, verify_proof},
        poly::{
    ...
    #[test]
    fn bench_packed_multi_keccak_circuit_prover_single() {
    ...
    #[cfg_attr(not(feature = "benches"), ignore)]
    #[test]
    fn bench_packed_multi_keccak_circuit_prover_leader() {
        let setup_prfx = crate::constants::SETUP_PREFIX;
        let proof_gen_prfx = crate::constants::PROOFGEN_PREFIX;
        let proof_ver_prfx = crate::constants::PROOFVER_PREFIX;
        // Unique string used by bench results module for parsing the result
    ...
    #[cfg_attr(not(feature = "benches"), ignore)]
    fn bench_packed_multi_keccak_circuit_worker(worker_index: usize) {
        let setup_prfx = crate::constants::SETUP_PREFIX;
        let proof_gen_prfx = crate::constants::PROOFGEN_PREFIX;
        let proof_ver_prfx = crate::constants::PROOFVER_PREFIX;
        // Unique string used by bench results module for parsing the result
        ...
        // Initialize the proving key
        let vk: halo2_proofs::plonk::VerifyingKey<G1Affine> = keygen_vk(&general_params, &circuit).expect("keygen_vk should not fail");
        let pk = keygen_pk(&general_params, vk, &circuit).expect("keygen_pk should not fail");
        // Create a proof
        let mut transcript = Blake2bWrite::<_, G1Affine, Challenge255<_>>::init(vec![]);
