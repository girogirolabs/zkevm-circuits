...
usage() {
  echo "Usage: $0 [--profile dev|release] <setup|prove|prove-local|verify> [prover_index]"
  exit 1
}
...
if [[ "$COMMAND" == "prove" ]]; then
  cargo run --package distributed-prover --profile "$PROFILE" "$COMMAND" "$PROVER_INDEX"
else
  cargo run --package distributed-prover --profile "$PROFILE" "$COMMAND"
fi
